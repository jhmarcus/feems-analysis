...
def setup_graph(n_rows=8, 
    ...
    anisotropy_scaler : float
        scaler on horizontoal edge weights to create an anisotropic simulation
    ...
    """    
    graph = nx.generators.lattice.triangular_lattice_graph(n_rows - 1, 2 * n_columns - 2, with_positions=True)
    ...
    for i in graph.nodes:
        ...
        if x <= barrier_startpt: 
            graph.nodes[i]["sample_size"] = 2 * np.random.binomial(n_samples_per_node, corridor_left_prob)
        elif x >= barrier_endpt:
            graph.nodes[i]["sample_size"] = 2 * np.random.binomial(n_samples_per_node, corridor_right_prob)      
        else:
            graph.nodes[i]["sample_size"] = 2 * np.random.binomial(n_samples_per_node, barrier_prob)
...
def simulate_genotypes(graph, chrom_length=1, mu=1e-3, n_e=1, target_n_snps=1000, n_print=50):
    ...
    chrom_length : float
        length of the chromsome to simulate from
    ...
    # population config
    population_configurations = [msprime.PopulationConfiguration(sample_size=sample_sizes[i]) for i in range(d)] 
